...
router.get('/', (req, res, next) => {
  let { location, day, visitor, certification, farmType, activityType, public, language } = req.query;
  console.log('req.query', location, day, visitor, certification, farmType, activityType, public, language)
  ...
  const allCertifications = Host.schema.path("certifications").caster.enumValues;
  ...
  let query = {};
  if (location === '' && day === '' && visitor === '' && certification === '' && farmType === '' && activityType === '' && public ==='' && language === ''){
Host.find({})
.populate('userId')
.then(resultats => {
  console.log("resultats", resultats.length)
   res.render('recherche/recherche', { resultats, farmTypes, activitiesType, allCertifications, cities, publics, languages })
 }).catch(err => next(err))
  } else {
